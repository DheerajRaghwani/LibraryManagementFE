const baseUrl = "https://librarymanagement-jbdq.onrender.com/API/TempBookIssue";

function clearForm() {
    [
        "sno", "tempId", "referenceNo", "bookName", "noOfBookIssue", "noOfBookReturn", "issueBy",
        "dateOfIssue", "dateOfReturn", "fine", "anyUpdate",
    ].forEach(id => document.getElementById(id).value = "");
}

// Add
async function addTempBookIssue() {
    const issue = getTempBookIssueInput(false);
    try {
        ...
        alert(response.ok ? "Temp Book Issue added successfully" : "Failed to add");
        clearForm();
    } catch (err) {
...
// Update
async function updateTempBookIssue() {
    const issue = getTempBookIssueInput(true);
    const response = await fetch(baseUrl, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(issue)
    });
    alert(response.ok ? "Temp Book Issue updated successfully" : "Update failed");
    clearForm();
}

// Delete
async function deleteTempBookIssue() {
...
// View all
async function getAllTempBookIssues() {
    ...
    if (response.ok) {
        ...
        newWindow.document.write(`
            <html>
            <head>
                <title>All Temp Book Issues</title>
                <style>
                    body {
...
function getTempBookIssueInput(includeSno = true) {
    const data = {
        TempId: parseInt(document.getElementById("tempId").value),
...
function fillTempBookIssueInput(data) {
    ...
    document.getElementById("anyUpdate").value = data.anyUpdate;
}
